...
public ResponseEntity<LeaveForm> managerApplyLeave(
    ...
    public ResponseEntity<List<LeaveForm>> getLeaveHistoryByManager(@RequestParam Long managerId) {
        List<LeaveForm> leaveHistory = leaveService.getLeaveHistoryByManager(managerId);
        if (leaveHistory.isEmpty()) {
            return new ResponseEntity<>(HttpStatus.NO_CONTENT); // Return 204 if no leave history is found
        }
